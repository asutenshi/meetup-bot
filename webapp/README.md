...
- **`initData` протухает через 24 ч — экран `expired` после долгого простоя.**
  `initData` фиксируется при запуске Mini App и кэшируется в `sessionStorage`
  (`@tma.js`). Свёрнутый (не закрытый) Mini App Telegram Desktop/iOS держат в
  фоне сутками; вернувшись к нему, пользователь шлёт `auth_date` суточной
  давности, бэкенд отвечает `401 expired` (`webapp_init_data_max_age`, TZ §3.2),
  форма (`event-form/EventForm.tsx`) показывает «Не удалось открыть форму. Код:
  expired». Подпись валидна — протухает только возрастная проверка. Лечится
  полным закрытием Mini App (свайп / «Закрыть», не «Свернуть») и повторным
  открытием формы через `/edit_event`. `location.reload()` не помогает — вебвью
  перезагружает тот же URL с тем же хешем. Планы по смягчению (подъём `max_age`,
  отдельный код `clock_skew`, доработка текста и кнопки закрытия) —
  `docs/TASKS.md` → «Технический долг».
