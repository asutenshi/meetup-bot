...
- **`initData` не обновляется — `auth_date` заморожен на первом запуске.**
  Telegram-клиенты кэшируют launch-параметры: `auth_date` не меняется ни при
  сворачивании, ни при кнопке «Закрыть», ни при полном перезапуске Telegram, на
  всех платформах (проверено 2026-09-08). `@tma.js` держит `initData` только в
  `sessionStorage` (перезапуск его стирает) — значит кэширует сам клиент.
  Получить свежую подпись пользователь не может.
  Поэтому возрастная проверка `auth_date` на бэкенде **не блокирует**: подпись
  HMAC валидна всегда, а `auth_date` лишь окно replay. Просрочка старше
  `WEBAPP_INIT_DATA_MAX_AGE` (дефолт 7 сут) → WARNING `initData stale but
  accepted` в логах app, запрос проходит. Строгий режим — `401 expired` —
  включается `WEBAPP_INIT_DATA_REJECT_STALE=true` (TZ §3.2).
  Экраны-заглушки (`loadErrorText` / `STALE_SESSION_MESSAGE` в
  `src/api/events.ts`, кнопка «Закрыть» → `closeMiniApp()`) остаются для
  `clock_skew` (`auth_date` в будущем сверх допуска — рассинхрон часов сервера,
  жёсткий `401`) и для строгого режима.
