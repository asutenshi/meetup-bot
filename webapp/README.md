...
- **Форма создания мероприятия** (TASKS.md 2.5) — `src/event-form/`. Своя
  ...
  - **Дата и время — два поля.** «Когда» разбито на дату и время. Дата —
    ...
    полноценно). Время — видимый `<input type="time">`. Единый
    `datetime-local` не подошёл: на десктопе `showPicker()` открывает только
    календарь, а спрятанные сегменты времени недоступны. `color-scheme` на
    `:root` (`theme/tokens.css`) привязан к теме Telegram, чтобы нативные
    виджеты не были светлыми на тёмном фоне. Склейка/разбор `datetime-local` ↔
    поля ↔ ISO — `event-form/datetime.ts` (покрыто тестами).
